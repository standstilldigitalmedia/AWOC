...
var default_recipe_list: Dictionary
var slots_dictionary: Dictionary

func create_material_albedo():
	var first_material: AWOCMaterial = load(ResourceUID.get_id_path(recipe_list_dictionary["first slot"].material_resource_reference.resource_uid))
...
func populate_slots_dictionary():
	slots_dictionary = {}
	var counter: int = 0
	for slot in awoc_resource.slots_dictionary:
		slots_dictionary[slot] = counter
		counter += 1
...
func _ready():
	default_recipe_list = awoc_resource.get_default_recipe_dictionary()
	for recipe in default_recipe_list:
		recipe_list_dictionary[recipe] = default_recipe_list[recipe]
	populate_slots_dictionary()
